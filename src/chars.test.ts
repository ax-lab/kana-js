...
describe('chars', () => {
	...
	describe('chars.getCharInfo', () => {
		...
		test('should support CharKind.OTHER_WORD for lowercase letters', () => {
			...
			const lines = [
				'µßàáãäåæçèéëìíîïðñòóõöøùúüýþÿăąćĉċčďđĕėęěĝğġģĥħĩĭįıĳĵķĸĺļľŀłńņňŉŋŏőœŕŗřśŝşšţťŧũŭůűųŵŷźżž',
		...
		test('should support CharKind.OTHER_WORD for other digits', () => {
			const lines = [
				...
				'ᛮᛯᛰⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫⅬⅭⅮⅯⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹⅺⅻⅼⅽⅾⅿↀↁↂↅↆↇↈ〇〡〢〣〤〥〦〧〨〩〸〹〺𐍁𐍊𐏑𐏒𐏓𐏔𐏕',
